...
public class ProfileApplication : IProfileApplication
{
    ...
    public ProfileApplication(IProfileRepository repository, IAccountUnitOfWork unitOfWork, IWebHostEnvironment hostingEnvironment = null)
    {
    ...
    public async Task<OperationResult> CreateProfileAsync(CreateProfileCommand command)
    {
        var operation = new OperationResult();

        var exists = await _repository.IsExistsAsync(x => x.UserId == command.UserId);
        if (exists)
            return operation.Failed("برای این کاربر قبلاً پروفایل ثبت شده است.");

        var avatarPath = await UploadAvatarAsync(command.AvatarFile);
    ...
    public async Task<OperationResult> EditProfileAsync(EditProfileCommand command)
    {
        var operation = new OperationResult();
        var profile = await _repository.GetAsync(command.Id);
    ...
    public async Task<ProfileViewModel?> GetProfileByUserIdAsync(long userId)
    {
    ...
    {
        var folder = Path.Combine(_hostingEnvironment.WebRootPath, "uploads", "avatars");
