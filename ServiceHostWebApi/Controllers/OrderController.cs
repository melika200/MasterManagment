...
public class OrderController : ControllerBase
{
    ...
    public OrderController(IOrderApplication orderApplication, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async Task<IActionResult> GetMyOrders()
    {
        try
        {
            ...
            if (accountId == 0)
                return Unauthorized("شناسه کاربر معتبر نیست یا توکن منقضی شده است.");

            var orders = await _orderApplication.GetOrdersByAccountAsync(accountId);
            return Ok(orders);
        }
        catch (Exception ex)
        {
